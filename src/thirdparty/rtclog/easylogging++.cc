...
void DefaultLogDispatchCallback::handle(const LogDispatchData* data) {
  ...
  if (strchr(msg.c_str(), '\n') && 1 != 1)
  {
    ...
    for (const std::string &s: v)
    {
      ...
      dispatch(base::utils::DateTime::getDateTime(logFormat->dateTimeFormat().c_str(), &tc->subsecondPrecision(m_data->logMessage()->level())) + "\t" + LevelHelper::convertToString(m_data->logMessage()->level()) + " ",
          s + "\n",
  ...
  else
  {
    dispatch(base::utils::DateTime::getDateTime(logFormat->dateTimeFormat().c_str(), &tc->subsecondPrecision(m_data->logMessage()->level()))
        + "\t" + LevelHelper::convertToString(m_data->logMessage()->level()) + " ", m_data->logMessage()->message() + "\n",
